...
funcaoOrdenarFilme funcoesFilme[6] = {
  ordenarPorTituloCrescente,
  ordenarPorTituloDecrescente,
  ordenarPorDuracao,
  ordenarPorGenero,
  ordenarPorNota,
...
char *getClassificacaoFormatada(int classificacao) {
  ...
  classificacao ? 
    snprintf(info, totalCaracteres, "%d+", classificacao) : 
    snprintf(info, totalCaracteres, "L");

  return info;
}

// Funçaõ para mostrar formatado as informaçõs do filme
...
void mostrarInfoFilmePopular(ListaPopulares *filme){
    printf("Titulo: %s\n", filme->titulo);
    printf("ID: %s\n", filme->id);
